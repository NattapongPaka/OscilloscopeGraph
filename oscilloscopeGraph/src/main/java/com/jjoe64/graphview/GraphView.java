...
abstract public class GraphView extends LinearLayout {
	...
	private class GraphViewContentView extends View {
		...
		//Modify
		private float lastSerialAllEventX = 0;
		private float lastSerialAllEventY = 0;
		
		private float lastSerial1EventX = 0;
		private float lastSerial1EventY = 0;
		
		private float lastSerial2EventX = 0;
		private float lastSerial2EventY = 0;
		...
		public GraphViewContentView(Context context) {
			...
			// Path Effect
			//mEffects = new DashPathEffect(new float[] {10, 15, 15, 15}, 3);
		}
		...
		protected void onDraw(Canvas canvas) {
			...
			if (labelTextHeight == null || horLabelTextWidth == null) {
				...
				Log.i("TextSizeX", String.valueOf(testX));
			...
			Log.i("graphheight", String.valueOf(graphheight));
			
			graphwidth = width;
			Log.i("graphwidth", String.valueOf(graphwidth));
			...
			if (graphViewStyle.getGridStyle().drawHorizontal()) {
				...
				Log.i("LabelVerticalLength", String.valueOf(vers));
				for (int i = 0; i < verlabels.length; i++) {
					
					float y = ((graphheight / vers) * i) + border;
					Log.i("TextSizeY", String.valueOf(y));
					//แก้สี  Horizontal lines
					if(i == (verlabels.length/2)){
						paint.setColor(Color.RED);
						posCenter = i;
						Log.i("Center_point", String.valueOf(y));
					}else{
...
//				String serialDes = graphSeries.get(0).description;
//				if(serialDes.contains(SettingDialog.serial1)){
//					Log.i("serialDes1", serialDes);
//					for (int i = 0; i < graphSeries.size(); i++) {
//						drawSeries(canvas, _values(i), graphwidth, graphheight, border,
//								minX, minY, diffX, diffY, horstart,
//								graphSeries.get(i).style,lastSerial1EventX,lastSerial1EventY,SettingDialog.serial1ID,onTouch);
//					}
//				}
//				if(serialDes.contains(SettingDialog.serial2)){
//					Log.i("serialDes2", serialDes);
//					for (int i = 0; i < graphSeries.size(); i++) {
//						drawSeries(canvas, _values(i), graphwidth, graphheight, border,
//								minX, minY, diffX, diffY, horstart,
//								graphSeries.get(i).style,lastSerial2EventX,lastSerial2EventY,SettingDialog.serial2ID,onTouch);
//					}
//				}						
//			}
//			else{
...
//			}
			...
			for (int i = 0; i < graphSeries.size(); i++) {
				String serial = graphSeries.get(i).description;
				if (serial.contains(SettingDialog.serial1)) {
					
					drawSeries(canvas, _values(i), graphwidth, graphheight,
							border, minX, minY, diffX, diffY, horstart,
							graphSeries.get(i).style, lastSerial1EventX,
							lastSerial1EventY, SettingDialog.serial1ID,
							onTouch,posCenter);
				} else if (serial.contains(SettingDialog.serial2)) {
					drawSeries(canvas, _values(i), graphwidth, graphheight,
							border, minX, minY, diffX, diffY, horstart,
							graphSeries.get(i).style, lastSerial2EventX,
							lastSerial2EventY, SettingDialog.serial2ID,
							onTouch,posCenter);
				}
			}			
			...
			if (!handled) {
				...
				if (event.getAction() == MotionEvent.ACTION_MOVE){
					if(graphSeries.size() == 1){	
						...
						if (serialDes.contains(SettingDialog.serial1)) {
							Log.i("serialDes1", serialDes);
							lastSerial1EventX = event.getX();
							lastSerial1EventY = event.getY();
						} else if (serialDes.contains(SettingDialog.serial2)) {
							Log.i("serialDes2", serialDes);
							lastSerial2EventX = event.getX();
							lastSerial2EventY = event.getY();
						}
						//onTouch = true;
					}
					
					else{
						lastSerialAllEventX = event.getX();
						lastSerialAllEventY = event.getY();
						
						lastSerial1EventX = lastSerialAllEventX;						
						lastSerial2EventX = lastSerialAllEventX;
						
						//onTouch = true;
					}
					Log.i("ACTION_MOVE", "X" + lastSerialAllEventX + " : Y" + lastSerialAllEventY);
					handled = true;
					onTouch = true;
				}
	...
	protected void drawHorizontalLabels(Canvas canvas, float border,
		...
		for (int i = 0; i < horlabels.length; i++) {
			...
			if (graphViewStyle.getGridStyle().drawVertical()) { 
				// vertical lines
				if(i == (horlabels.length/2)){
					paint.setColor(Color.RED);
				}else{
					paint.setColor(graphViewStyle.getGridColor());
				}
				Log.i("VerticalLine", String.valueOf(x));
			...
			if (showHorizontalLabels) {
				...
				for (int li = 0; li < lines.length; li++) {
					...
					Log.i("VerticalText", String.valueOf(y));
	...
	abstract protected void drawSeries(Canvas canvas,
			...
			GraphViewSeriesStyle style,float lastActionEventX,float lastActionEventY,int serialID,boolean isTouch,float center);
	...
	private String[] generateHorlabels(float graphwidth) {
		...
		if (numLabels < 0) { // automatic
			...
			Log.i("generateHorlabels_numLabels", String.valueOf(numLabels));
		}
		...
		for (int i = 0; i <= numLabels; i++) {
			...
			Log.i("generateHorlabels", String.valueOf(labels[i]));
		}
		return labels;
	}
	...
	synchronized private String[] generateVerlabels(float graphheight) {
		...
		if (numLabels < 0) { // automatic
			...
			Log.i("generateVerlabels_numLabels", String.valueOf(numLabels));
			if (numLabels == 0) {
				Log.w("GraphView",
		...
		for (int i = 0; i <= numLabels; i++) {
			...
			Log.i("generateVerlabels", String.valueOf(labels[numLabels - i]));
		}
		return labels;
	}
	...
	public void redrawAll() {
		...
		// graphViewContentView.postInvalidate();
	}
	...
	public void scrollToEnd() {
		...
		// don't clear labels width/height cache so that the display is not flickering
		if (!staticVerticalLabels)
			verlabels = null;
		if (!staticHorizontalLabels)
			horlabels = null;
